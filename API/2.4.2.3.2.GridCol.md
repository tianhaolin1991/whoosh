# GridCol

栅格子组件，必须作为栅格容器组件( GridRow )的子组件使用。

## Component Constructors

### public init(span!: Int32 = 1, offset!: Int32 = 0, order!: Int32 = 0, child!: () -> Unit)

```cangjie
public init(span!: Int32 = 1, offset!: Int32 = 0, order!: Int32 = 0, child!: () -> Unit)
```
功能： 创建一个栅格布局子组件。

**Parameters:**

| type | required | description | name | default |
| --- | --- | --- | --- | --- |
| Int32 | 否 | 命名参数。 栅格子组件占用栅格容器组件( GridRow )的列数。 span为0表示该元素不参与布局计算，即不会被渲染。 | span | 1 |
| Int32 | 否 | 命名参数。 栅格子组件相对于前一个栅格子组件偏移的列数。 | offset | 0 |
| Int32 | 否 | 命名参数。 元素的序号，根据栅格子组件的序号，从小到大对栅格子组件做排序。 | order | 0 |
| ()->Unit | 是 | 命名参数。 GridCol容器的子组件。 | child | - |

### public init( span!: GridColColumnOption, offset!: GridColColumnOption, order!: GridColColumnOption, child!: () -> Unit )

```cangjie
public init( span!: GridColColumnOption, offset!: GridColColumnOption, order!: GridColColumnOption, child!: () -> Unit )
```
功能： 创建栅格子组件。

**Parameters:**

| type | required | description | name | default |
| --- | --- | --- | --- | --- |
| GridColColumnOption | 是 | 命名参数。 占用列数。 | span | - |
| GridColColumnOption | 是 | 命名参数。 相对于前一个栅格子组件偏移的列数。 | offset | - |
| GridColColumnOption | 是 | 命名参数。 元素的序号，根据栅格子组件的序号，从小到大对栅格子组件做排序。 | order | - |
| ()->Unit | 是 | 命名参数。 GridCol 容器的子组件。 | child | - |


## Functions

### public func gridColOffset(offset: Int32): This

```cangjie
public func gridColOffset(offset: Int32): This
```
功能： 设置相对于前一个栅格子组件偏移的列数。

**Parameters:**

| type | required | description | name | default |
| --- | --- | --- | --- | --- |
| Int32 | 是 | 相对于前一个栅格子组件偏移的列数。 取值为大于0的整数，初始值：0。 | offset | - |

### public func gridColOffset(offset: GridColColumnOption): This

```cangjie
public func gridColOffset(offset: GridColColumnOption): This
```
功能： 设置相对于前一个栅格子组件偏移的列数。

**Parameters:**

| type | required | description | name | default |
| --- | --- | --- | --- | --- |
| GridColColumnOption | 是 | 相对于前一个栅格子组件偏移的列数。 | offset | - |

### public func offset(offset: Int32): This

```cangjie
public func offset(offset: Int32): This
```
功能： 设置相对于前一个栅格子组件偏移的列数。

**Parameters:**

| type | required | description | name | default |
| --- | --- | --- | --- | --- |
| Int32 | 是 | 相对于前一个栅格子组件偏移的列数。 取值为大于0的整数，初始值：0。 | offset | - |

### public func offset(offset: GridColColumnOption): This

```cangjie
public func offset(offset: GridColColumnOption): This
```
功能： 设置相对于前一个栅格子组件偏移的列数。

**Parameters:**

| type | required | description | name | default |
| --- | --- | --- | --- | --- |
| GridColColumnOption | 是 | 相对于前一个栅格子组件偏移的列数。 | offset | - |

### public func order(order: Int32): This

```cangjie
public func order(order: Int32): This
```
功能： 设置元素的序号，根据栅格子组件的序号，从小到大对栅格子组件做排序。

**Parameters:**

| type | required | description | name | default |
| --- | --- | --- | --- | --- |
| Int32 | 是 | 元素的序号。 取值为大于0的整数，初始值：0。 | order | - |

### public func order(order: GridColColumnOption): This

```cangjie
public func order(order: GridColColumnOption): This
```
功能： 设置元素的序号，根据栅格子组件的序号，从小到大对栅格子组件做排序。

**Parameters:**

| type | required | description | name | default |
| --- | --- | --- | --- | --- |
| GridColColumnOption | 是 | 元素的序号。 | order | - |

### public func span(span: Int32): This

```cangjie
public func span(span: Int32): This
```
功能： 设置占用列数。

**Parameters:**

| type | required | description | name | default |
| --- | --- | --- | --- | --- |
| Int32 | 是 | 占用列数。 取值为大于0的整数，初始值：0。 span为0表示该元素不参与布局计算，即不会被渲染。 | span | - |

### public func span(span: GridColColumnOption): This

```cangjie
public func span(span: GridColColumnOption): This
```
功能： 设置占用列数。

**Parameters:**

| type | required | description | name | default |
| --- | --- | --- | --- | --- |
| GridColColumnOption | 是 | 占用列数。 span为0表示该元素不参与布局计算，即不会被渲染。 | span | - |


## Structs

### GridColColumnOption

struct GridColColumnOption
```cangjie
public struct GridColColumnOption { public GridColColumnOption( xs!: Int32 = 12, sm!: Int32 = 12, md!: Int32 = 12, lg!: Int32 = 12, xl!: Int32 = 12, xxl!: Int32 = 12 ) }
```
功能： 用于自定义指定在不同宽度设备类型上，栅格子组件占据的栅格数量单位。



## Examples

```cangjie
package ohos_app_cangjie_entry
import kit.UIKit.*
import ohos.state_macro_manage.*

@Entry
@Component
class EntryView {
 var bgColors: Array<Color> = [Color.RED, Color.GREEN, Color.GRAY, Color.RED, Color.GREEN, Color.BLUE, Color.GRAY]
 var currentBp: String = ""
 func build() {
 Column {
 //GridRow设置中一行有5列
 GridRow(
 columns: 5,
 gutter: GutterOption(x: 5.vp, y: 10.vp),
 breakpoints: BreakPoints(
 value: [400.vp, 600.vp, 800.vp],
 reference: BreakpointsReference.WindowSize
 ),
 direction: GridRowDirection.GridRowRow
 ) {
 //GridCol栅格子组件，必须作为栅格容器组件(GridRow)的子组件使用。
 //构造第1个GridCol子组件，栅格占2列，为第1,2列
 GridCol() {
 Row() {
 Text("GridCol 1: span=2")
 .fontColor(Color.GRAY)
 .fontSize(12)
 }
 .width(100.percent)
 .height(40.vp)
 }
 .borderColor(Color.BLUE)
 .borderWidth(2.vp)
 .span(2)
 //构造第2个GridCol子组件，栅格占2列，离上一个栅格的偏移列数为1列，因此该栅格占第4.5列
 GridCol() {
 Row() {
 Text("GridCol 2: span=2, offset=1")
 .fontColor(Color.GRAY)
 .fontSize(12)
 }
 .width(100.percent)
 .height(40.vp)
 }
 .borderColor(Color.GREEN)
 .borderWidth(2.vp)
 .span(2)
 .gridColOffset(1)
 //构造第3个GridCol子组件，栅格占1列，距离上一个栅格偏移列数为2列，由于换行，因此该栅格占下一行的第3列
 GridCol() {
 Row() {
 Text("GridCol 3: span=1, offset=2")
 .fontColor(Color.GRAY)
 .fontSize(11)
 }
 .width(100.percent)
 .height(40.vp)
 }
 .borderColor(Color.RED)
 .borderWidth(2.vp)
 .span(1)
 .gridColOffset(2)

 //循环渲染栅格子组件
 ForEach(
 bgColors,
 itemGeneratorFunc: {
 color: Color, index: Int64 => GridCol() {
 Row().height(20.vp)
 }
 .borderWidth(2.vp)
 .borderColor(color)
 .span(GridColColumnOption(xs: 12, sm: 12, md: 12, lg: 12, xl: 12, xxl: 12))
 .id("my_GridCol")
 }
 )
 }
 .width(100.percent)
 .height(100.percent)
 .onBreakpointChange({
 bp => currentBp = bp
 })
 .alignItems(ItemAlign.Center)
 }
 .margin(left: 10, right:10, top: 5, bottom: 5)
 .height(300)
 }
}
```

